...
function render() {
	...
	// Draw each part
	// The 3 parts are in base, lower, and upper.
	// Each part is drawn with drawElements from .start with .count vertices
	// Each part will need to incorporate its position and rotation into the
	// model-view matrix being built then send the updated mv matrix to the GPU.
	// The model-view matrix is not reset between parts.
	let mv = mat4(); // start with no transformation
	// Drawing Base
	mv = mult(mv, translate(base.position[0], base.position[1], base.position[2]));
	mv = mult(mv, rotateZ(base.rotation[2]));
	mv = mult(mv, rotateY(base.rotation[1]));
	mv = mult(mv, rotateX(base.rotation[0]));
	gl.uniformMatrix4fv(model_view_loc, false, flatten(mv));
	gl.drawElements(gl.TRIANGLE_STRIP, base.count, gl.UNSIGNED_SHORT, 2*base.start);
	// TODO: Lower Arm
	// TODO: Upper Arm
