// This is a WebGL example that draws a cube with a checkboard texture with an overlaid sinusoidal
// function.
...
window.addEventListener('load', function init() {
	...
	let img2 = create_sinusodial_image(512);
	let texCoords = [
	...
	let fragShdr = compileShader(gl, gl.FRAGMENT_SHADER, `
		...
		void main() {
			...
			vec4 color = texture2D(texture1, fTexCoord)*texture2D(texture2, fTexCoord)*material_color;
...
/**
 * Create a sinusoidal image. The image will be texSize-by-texSize pixels.
 */
function create_sinusodial_image(texSize) {
	let img = new ImageData(texSize, texSize);
	let data = img.data;
	for (let i = 0; i < texSize; ++i) {
		for (let j = 0; j < texSize; ++j) {
			let off = 4*(i*texSize+j);
			let c = 127.5 + 127.5*Math.sin(4*Math.PI*(i+j)/texSize);
			data[off] = data[off+1] = data[off+2] = c;
			data[off+3] = 255;
		}
	}
