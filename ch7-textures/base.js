// This is a WebGL example that draws a cube with basic lighting. Has some functions for dealing
// with textures.
...
window.addEventListener('load', function init() {
	...
	let fragShdr = compileShader(gl, gl.FRAGMENT_SHADER, `
		...
		const vec4 color_a = vec4(0.0, 0.2, 0.0, 1.0);
		const vec4 color_d = vec4(0.0, 1.0, 0.0, 1.0);
		const vec4 color_s = vec4(1.0, 1.0, 1.0, 1.0);
		...
		void main() {
			...
			gl_FragColor = color_a + d*color_d + s*color_s;
...
/**
 * Load a texture onto the GPU into the TEXTURE0 slot. The image must be power-of-two sized image
 * using RGBA with uint8 values. The image will be flipped vertically and will support mipmapping.
 */
