// This is a WebGL example that draws a cube that has ambient, diffuse, and specular lighting.
...
// Uniform locations
let model_view_loc, projection_loc, color_loc, light_loc, light_src_loc, ka_loc, kd_loc, ks_loc, shininess_loc;
...
// HTML Elements for the light source
...
window.addEventListener('load', function init() {
	...
	let vertShdr = compileShader(gl, gl.VERTEX_SHADER, `
		...
		void main() {
			...
			// NOTE: this assumes that the model-view matrix does not apply to the light source
			// If it should, then change the next line to L = model_view*light_src;
			L = light_src;
			if (light_src.w == 1.0) {
			...
			// NOTE: this assumes viewer is at <0,0,0> and in model coordinates
	...
	gl.uniform4fv(light_src_loc, vec4(1, 1, -1, 0));
	gl.uniform1f(ka_loc, 0.2);
	gl.uniform1f(kd_loc, 1.0);
	gl.uniform1f(ks_loc, 1.0);
	gl.uniform1f(shininess_loc, 20.0);
	...
	// Add listeners for the material properties
	document.getElementById('ka').addEventListener('change', onChangeKa);
	document.getElementById('kd').addEventListener('change', onChangeKd);
	document.getElementById('ks').addEventListener('change', onChangeKs);
	document.getElementById('shininess').addEventListener('change', onChangeShininess);

	// Add listeners for the light source
...
/**
 * When the Kd is changed update the coefficient on the GPU
 */
function onChangeKd() {
	gl.uniform1f(kd_loc, +this.value);
}

/**
 * When the Ks is changed update the coefficient on the GPU
 */
function onChangeKs() {
	gl.uniform1f(ks_loc, +this.value);
}

/**
 * When the shininess is changed update the value on the GPU
 */
function onChangeShininess() {
	gl.uniform1f(shininess_loc, +this.value);
}
