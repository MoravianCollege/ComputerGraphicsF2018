// This is a WebGL example that draws a circle using a triangle fan.
 ...
 * For use with triangle fan.
 */
function circle(c, r, n, verts) {
	let theta = 2*Math.PI/n;
	verts.push(c);
	verts.push(vec2(c[0]+r, c[1]));
	for (let i = 1; i <= n; ++i) {
		verts.push(vec2(c[0]+Math.cos(i*theta)*r, c[1]+Math.sin(i*theta)*r));
	}
}
