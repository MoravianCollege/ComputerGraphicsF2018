...
window.addEventListener('load', function init() {
	...
	`);
	let fragShdr_id = compileShader(gl, gl.FRAGMENT_SHADER, `
		precision mediump float;
		uniform int id;
		void main() {
			gl_FragColor.r = float(id)/255.0;
			gl_FragColor.g = 0.0;
			gl_FragColor.b = 0.0;
			gl_FragColor.a = 1.0;
		}
	...
	program_id = linkProgram(gl, [vertShdr, fragShdr_id]);
...
function get_object_id(x, y) {
	...
	// Read the pixel under the mouse click. The color should contain the ID
	// in the red channel and the alpha channel tells us if it is completely empty
	// or not. If empty, return null. Otherwise return the ID.
	let buf = new Uint8Array(4);
	gl.readPixels(x, y, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, buf);
	if (buf[3] === 0) { return null; } // x,y is in background
	return buf[0];
}
...
function onClick(evt) {
	...
	if (evt.altKey) {
		...
		x = (x / gl.canvas.width) * 3.8 - 1.9;
		y = (y / gl.canvas.height) * 3.8 - 1.9;
		obj[2] = vec3(x, y, -5);
		objects.push(obj);
	} else {
		let id = get_object_id(x, y);
		if (id !== null) {
			if (evt.shiftKey) {
				objects[id][0] = rand_int(0, 3);
			} else if (evt.metaKey) {
				objects.splice(id, 1);
			} else {
				objects[id][1] = rand_color();
			}
		}
	}
}
