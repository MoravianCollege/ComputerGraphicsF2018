// This is a WebGL example that draws a cube with a bumb map.
...
window.addEventListener('load', function init() {
	...
	let fragShdr = compileShader(gl, gl.FRAGMENT_SHADER, `
		...
		const float ka = 0.2, kd = 0.8, ks = 0.1, shininess = 10.0;
		const vec4 material = vec4(0.0, 1.0, 0.0, 1.0);
...
function create_bump_map(texSize) {
	...
	for (let i = 0; i < texSize; i++) {
		for (let j = 0; j < texSize; j++) {
			if (i > texSize / 4 && i < 3*texSize/4 &&
				j > texSize / 4 && j < 3*texSize/4) {
				d[i*texSize+j] = 1;
			} else {
				d[i*texSize+j] = 0;
			}
